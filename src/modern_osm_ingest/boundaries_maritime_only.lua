...
function osm2pgsql.select_relation_members(relation)
    if relation.tags.boundary == 'maritime' then
        return { ways = osm2pgsql.way_member_geoms }
    end
end

-- Processa relations (boundaries maritime)
function osm2pgsql.process_relation(object)
    local tags = object.tags

    if not (tags.boundary == 'maritime') then
        return
    end
...
print(string.format("Configuração carregada: %s (border_type)", CONFIG.table_name)
